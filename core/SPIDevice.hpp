...
/* export */ namespace atmc
{
    ...
    {
    public:
        static InplaceAtomicSet<SPI_HandleTypeDef*, Config::SPIBusCount> txDone;
        static InplaceAtomicSet<SPI_HandleTypeDef*, Config::SPIBusCount> rxDone;
        static InplaceAtomicSet<SPI_HandleTypeDef*, Config::SPIBusCount> txrxDone;
        static_assert(std::atomic<SPI_HandleTypeDef*>::is_always_lock_free, "Atomic SPI handle must be lock-free.");

        static SpinLock busyLock;
        static InplaceSet<SPI_HandleTypeDef*, Config::SPIBusCount> busy;
    ...
    class SPIDevice final : public SerialInterfaceDevice
    {
        ...
        inline Task<HardwareStatus> rwMemory(uint16_t memAddr, Task<HardwareStatus> rw)
        {
    ...
    public:
        inline SPIDevice(FencedPointer<SPI_HandleTypeDef> hspi, GPIOPin csPin,
        ...
        private:
            ...
            inline AcquireGuard(FencedPointer<SPIDevice> hspi) : hspi(&*hspi)
            {
                this->hspi->select();
            }
        };
        ...
        private:
            ...
            inline AccessGuard(FencedPointer<SPI_HandleTypeDef> hspi) : hspi(&*hspi)
            { }
        };

        Task<AccessGuard> guardBus()
        {
        Retry:
            {
                LockGuard guard(SPIManager::busyLock);
        ...
        inline Task<HardwareStatus> readMemory(std::span<uint8_t> data)
        {
            auto accGuard = co_await this->guardBus();
            auto csGuard = this->acquire();
            co_return co_await this->readMemoryUnchecked(data);
        }
        inline Task<HardwareStatus> writeMemory(std::span<const uint8_t> data)
        {
            auto accGuard = co_await this->guardBus();
            auto csGuard = this->acquire();
            co_return co_await this->writeMemoryUnchecked(data);
        }
        inline Task<HardwareStatus> exchangeMemory(uint8_t dataRx[], uint8_t dataTx[], size_t dataSize)
        {
            auto accGuard = co_await this->guardBus();
            auto csGuard = this->acquire();
        ...
        inline Task<HardwareStatus> readMemory(uint16_t memAddr, std::span<uint8_t> data) override
        {
            return this->rwMemory(memAddr, this->readMemoryUnchecked(data));
        }
        inline Task<HardwareStatus> writeMemory(uint16_t memAddr, std::span<uint8_t> data) override
        {
